...
def preprocess_image(image_path, input_size=(640, 640)):
    ...
    # Convert to float16 if needed by the model
...
def post_process(predictions, conf_threshold=0.25, iou_threshold=0.45, input_size=(640, 640)):
    # Predictions are in the format (N, grid, grid, anchors, [x, y, w, h, conf, class_probs])
    ...
    # Calculate the confidence score for each box (conf * class probabilities)
    scores = conf * class_probs  # Shape: (N, grid, grid, anchors, num_classes)
    ...
    scores = scores.reshape(-1, scores.shape[-1])  # Flatten the scores for each box
    conf = conf.reshape(-1, 1)  # Flatten the confidence scores

    # Apply a threshold to filter out low-confidence boxes
    mask = scores.max(axis=1) > conf_threshold  # Only keep boxes above the confidence threshold
    boxes = boxes[mask]
    scores = scores[mask]
    conf = conf[mask]

    # Convert boxes back to the original image scale (from normalized to pixels)
    h, w = input_size
    boxes = boxes * np.array([w, h, w, h])  # Scale the boxes back to the original image size

    # Apply Non-Maximum Suppression (NMS)
    indices = cv2.dnn.NMSBoxes(boxes.tolist(), scores.max(axis=1).tolist(), conf_threshold, iou_threshold)

    # Collect results (boxes and their corresponding scores)
    results = []
    if len(indices) > 0:
        for idx in indices.flatten():
            x1, y1, x2, y2 = boxes[idx]
            score = scores[idx].max()  # Get the maximum score for the class
